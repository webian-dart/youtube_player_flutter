...
class _PlayPauseButtonState extends State<PlayPauseButton>
  ...
  void _playPauseListener() => _controller?.value.isPlaying ?? false
  ...
  Widget build(BuildContext context) {
    ...
    if ((_controller?.flags.autoPlay == false &&
            _controller?.value.isReady == false) ||
        _playerState == PlayerState.playing ||
        _playerState == PlayerState.paused) {
      return Visibility(
        visible: _playerState == PlayerState.cued ||
            _controller?.value.isPlaying == false ||
            _controller?.value.isControlsVisible == true,
        child: Material(
    ...
    if (_controller?.value.hasError ?? false) return const SizedBox();
